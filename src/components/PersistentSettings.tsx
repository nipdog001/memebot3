...
interface PersistentSettingsProps {
...
export default function PersistentSettings({ onSettingsChange, userTier, isAdmin }: PersistentSettingsProps) {
  ...
  const [serverSyncAttempted, setServerSyncAttempted] = useState(false);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.settings) {
          ...
          setServerSyncAttempted(true);
        }
      }
    } catch (error) {
      console.warn('⚠️ Could not fetch settings from server:', error);
    }
  };
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      try {
        const response = await fetch('/api/trading/settings', {
          ...
          headers: {
            ...
            'X-Device-ID': localStorage.getItem('memebot_device_id') || 'unknown',
        ...
        console.warn('⚠️ Could not save settings to server:', error);
      }
      ...
      try {
        ...
        console.warn('⚠️ Error syncing settings with WebSocket:', error);
      }
  ...
  const simulateSync = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Could not sync settings with server:', error);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-white">Persistent Trading Settings</h2>
      ...
      {activeTab === 'trading' && (
        <div className="space-y-6">
          ...
          <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
            <h3 className="text-lg font-bold text-white mb-4">🎯 Trading Mode Configuration</h3>
            ...
            <div className="space-y-4">
              <div className="bg-orange-600/20 border border-orange-600/30 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-orange-400">Training Mode</h4>
                      <p className="text-sm text-gray-300">
                        Keep AI in continuous learning mode for optimal performance
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => updateSetting('isTrainingMode', !settings.isTrainingMode)}
                    className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-semibold transition-all ${
                      settings.isTrainingMode 
                ...
                {settings.isTrainingMode && (
                  <div className="text-sm text-gray-300">
                    ✅ AI models are continuously learning and improving from market data
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="flex items-center space-x-3">
                    <button
                      onClick={() => updateSetting('isPaperTrading', true)}
                    ...
                    <button
                      onClick={() => updateSetting('isPaperTrading', false)}
                      disabled={userTier === 'basic'}
                      className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-all ${
                        !settings.isPaperTrading 
                          ? 'bg-red-600 text-white' 
                          : 'bg-gray-600 text-gray-300'
                      } ${userTier === 'basic' ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      <DollarSign className="h-4 w-4" />
                      <span>Live Trading</span>
                      {userTier === 'basic' && (
                        <span className="text-xs bg-orange-600 px-1 rounded">PRO+</span>
                      )}
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-3">
                    <input
                      type="range"
                      min="0.1"
                      max={limits.maxTradeSize === -1 ? "25" : "10"}
                      step="0.1"
                      value={settings.positionSize}
                      onChange={(e) => updateSetting('positionSize', parseFloat(e.target.value))}
                      className="flex-1"
                    />
                    <span className="text-white font-medium w-16">
                      {settings.positionSize.toFixed(1)}%
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateSetting('minTradeSize', parseFloat(e.target.value))}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">
                    ...
                    <span className="text-xs text-gray-500 ml-1">
                      (Limit: {formatLimit(limits.maxTradeSize)})
                    </span>
                  </label>
                  <input
                    type="number"
                    min={settings.minTradeSize}
                    max={limits.maxTradeSize === -1 ? undefined : limits.maxTradeSize}
        ...
        <div className="space-y-6">
          <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => updateSetting('syncSettings', !settings.syncSettings)}
                className={`p-1 rounded-full transition-all ${
                  settings.syncSettings ? 'bg-green-600' : 'bg-gray-600'
                }`}
              >
              ...
              <>
                <div className="bg-blue-600/20 border border-blue-600/30 rounded-lg p-4 mb-4">
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-300 mt-1">
                    ...
                    <span>Every {settings.syncFrequency} seconds</span>
                  </div>
